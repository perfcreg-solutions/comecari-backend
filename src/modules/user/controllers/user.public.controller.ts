...
import {
    UserPublicAddPhoneNumber,
    UserPublicLoginDoc,
    UserPublicSignUpDoc,
    UserPublicVerifyNumber,
...
import { UserAddPhoneNumber } from 'src/modules/user/dtos/user.add-phone.dto';
...
export class UserPublicController {
    ...
    @Post('/login')
    async login(
        @Body() { email, password }: UserLoginDto): Promise<IResponse> {
    ...
    @UserPublicAddPhoneNumber()
    @Response('user.addPhoneNumber')
    @Post('/add-number')
    async addPhoneNumber(
        @Body() { mobileNumber }: UserAddPhoneNumber): Promise<IResponse> {
        const mobileNumberExist: boolean = await this.userService.existByMobileNumber(mobileNumber);
        ...
        try {
            const sendToken: TermiiCreateDto = {
                mobileNumber
            }
            await this.termiiService.sendToken(sendToken);
            return;
        } catch (error: any) {
            console.log(error);
            throw new InternalServerErrorException({
    ...
    @UserPublicVerifyNumber()
    @Response('user.verifyPhoneNumber')
    @Post('/verify-number')
    async verifyToken(
        @Body() { 
            token
         }: TermiiVerifyDto): Promise<IResponse> {
        try {
            const verifyToken = {
                token
            }
            await this.termiiService.verifyToken(verifyToken);
            return;
        } catch (error: any) {
            console.log(error)
            throw new InternalServerErrorException({
